...
[BepInPlugin(PluginId, ModName, Version)]
...
public class Plugin : BaseUnityPlugin
{
    ...
    private void Awake()
    {
        ...
        Config.Bind("General", "NexusID", 1039, "Nexus mod ID for updates");
        PrefabListInConfigSettings = Config.Bind("General", "PrefabsToChange", DefaultPrefabsConfigSetting,
                $"List of the prefab pieces to weatherproof.  Comma-separate each prefab name.  Default: {DefaultPrefabsConfigSetting}")
            .Value;
        HammersListInConfigSettings = Config.Bind("General", "Hammers", DefaultHammersConfigSetting,
                $"List of hammer tools whose recipes we'll search to find the pieces to update.  These are the prefab names for each hammer piece.  Comma-separate each prefab name.  Default: {DefaultHammersConfigSetting}")
            .Value;
        RemoveRainDamage = Config.Bind("General", "RemoveRainDamage", DefaultRainDamageConfigSetting,
            "Prevent the floor pieces from taking wear and tear damage from rain.").Value;

        Log.LogInfo(
            $"Initializing using config settings: PrefabsToChange:[{PrefabListInConfigSettings ?? "null"}], Hammers:[{HammersListInConfigSettings ?? "null"}], RemoveRainDamage:[{RemoveRainDamage}]");
        ...
        // Normalize them
        var prefabsNormalized = PrefabListInConfigSettings.Replace(" ", "");
        var hammersNormalized = HammersListInConfigSettings.Replace(" ", "");

        char[] seperators = { ',' };
        PrefabList = prefabsNormalized.Split(seperators, StringSplitOptions.RemoveEmptyEntries);
        HammerList = hammersNormalized.Split(seperators, StringSplitOptions.RemoveEmptyEntries);
    ...
    internal static void PatchFloor()
    {
        ...
        var cTotalFloorsUpdated = 0;
        var completeSuccess = true;
        foreach (var hammerName in HammerList)
        {
            var cFloorsUpdated = 0;
            completeSuccess &= UpdatePiecesInHammer(hammerName, out cFloorsUpdated);
            cTotalFloorsUpdated += cFloorsUpdated;
        }

        Debug.Log(
            $"[floorsareroofs]: Successfully updated {cTotalFloorsUpdated} floors.  Config setting has {PrefabList.Length} floors specified.");
        ...
        if (completeSuccess)
        {
            ...
            {
                Debug.Log(
                    "[floorsareroofs]: Floor update finished, but it looks like we couldn't find some of the floors you specified.  Please doublecheck your config settings.");
        ...
        } else
        {
            // Be helpful!
            Debug.LogError(
                "[floorsareroofs]: Floor update finished, but there were errors.  Please doublecheck your config settings.");
            LogConfigSettingHelp();
        }
    }
    ...
    {
        Debug.Log(
            "[floorsareroofs]: The config settings are case-sensitive, each item should be separated by a comma, and there should be no spaces before or after the commas.");

        if (HammersListInConfigSettings != DefaultHammersConfigSetting)
            Debug.Log(
                $"[floorsareroofs]: You have a non-default config setting for Hammers.  Default: {DefaultHammersConfigSetting}.  Your setting: {HammersListInConfigSettings}");
        if (PrefabListInConfigSettings != DefaultPrefabsConfigSetting)
            Debug.Log(
                $"[floorsareroofs]: You have a non-default config setting for PrefabsToChange.  Default: {DefaultPrefabsConfigSetting}.  Your setting: {PrefabListInConfigSettings}");
    }
    ...
    private static bool UpdatePiecesInHammer(string hammerName, out int cFloorsUpdated)
    {
        ...
        var hammer = ObjectDB.instance.GetItemPrefab(hammerName);
        if (hammer == null)
        {
        ...
        if (!hammer.TryGetComponent(out hammerItemDrop))
        {
            Debug.LogError($"[floorsareroofs]: Could not get itemdrop from hammer {hammerName}");
            return false;
        }

        var hammerPieceTable = hammerItemDrop?.m_itemData?.m_shared?.m_buildPieces;
        ...
        {
            Debug.LogError(
                $"[floorsareroofs]: Could not find any pieces in hammerPieceTable for hammer {hammerName}");
            return false;
        }

        //List<GameObject> floors = hammerPieceTable.m_pieces.FindAll(i => i?.name == "wood_floor" || i?.name == "wood_floor_1x1");
        var floors = hammerPieceTable.m_pieces.FindAll(i => PrefabList.Contains(i?.name));
        if (floors == null || floors.Count == 0)
        {
            Debug.Log(
                $"[floorsareroofs]: Could not find any of the specified floors in hammer piece {hammerName}.  Continuing...");
            return true;
        }

        foreach (var go in floors)
        {
            var successful = PatchAFloor(go);
            if (successful)
                cFloorsUpdated++;
        }

        Debug.Log(
            $"[floorsareroofs]: Finished with hammer {hammerName}.  Sucessfully updated {cFloorsUpdated} floors.");
    ...
    private static bool PatchAFloor(GameObject go)
    {
        ...
        for (var iChild = 0; iChild < tr.childCount; iChild++)
        {
            var trChild = tr.GetChild(iChild);
            ...
            {
                Debug.LogError(
                    $"[floorsareroofs]: Could not patch {go.name}.  Its collider has tag '{trChild.tag ?? "[null]"}' instead of 'leaky'");
            }
        }
        ...
        {
            Debug.Log(
                $"[floorsareroofs] Not updating {go.name}.m_noRoofWear because config setting RemoveRainDamage is false");
        }
